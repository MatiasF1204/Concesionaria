...
if (!$cod_revision || !is_numeric($cod_revision)) {
    ...
    // Verificamos si la revisión existe
    $stmt_validar = $conexion->prepare("SELECT 1 FROM revision WHERE cod_revision = ?");
    $stmt_validar->bind_param("i", $cod_revision);
    ...
    if ($resultado_validar->num_rows === 0) {
        ...
        $stmt_eliminar = $conexion->prepare("DELETE FROM revision WHERE cod_revision = ?");
        $stmt_eliminar->bind_param("i", $cod_revision);
        if ($stmt_eliminar->execute()) {
            $mensaje = "✅ Revisión eliminada correctamente.";
            $tipoMensaje = "exito";
        } else {
            $mensaje = "Error al eliminar revisión: " . $stmt_eliminar->error;
            $tipoMensaje = "error";
        }
        $stmt_eliminar->close();
    }
