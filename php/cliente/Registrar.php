...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$nomyape || !$dni || !$correo || !$direccion || !$ciudad || !$telefono || !$falta) {
    ...
    // ✅ Verificar si ya existe un cliente con ese DNI, correo o teléfono
    $verificacion = $conexion->prepare("SELECT * FROM cliente WHERE dni = ? OR correo = ? OR telefono = ?");
    ...
    if ($resultado->num_rows > 0) {
      ...
      if (!$stmt) {
        ...
        if ($stmt->execute()) {
          ...
          $tipoMensaje = "exito";
          $nomyape = $dni = $correo = $direccion = $ciudad = $telefono = $falta = ''; // Limpiar campos
        } else {
